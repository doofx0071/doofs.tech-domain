        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto">
          {features.map((feature) => {
            ...
            return (
              <div
                key={feature.text}
                className={`group relative flex items-start gap-4 p-5 border-2 bg-card transition-all duration-300 ${
                  feature.available 
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className={`font-semibold ${feature.available ? "text-foreground" : "text-muted-foreground"}`}>
                      {feature.text}
                    </h3>
                    {feature.soon && (
                      <span className="text-[10px] font-mono font-bold bg-accent text-accent-foreground px-1.5 py-0.5">
                        SOON
                      </span>
                    )}
                  </div>
