        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {useCases.map((useCase, index) => {
            ...
            return (
              <div
                key={useCase.title}
                className="group bg-card border-2 border-border p-6 hover:border-primary/50 hover:-translate-y-1 transition-all duration-300 hover:shadow-lg"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                {/* Icon container */}
                <div className={`w-12 h-12 ${useCase.iconBg} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300`}>
